...
void CommandBufferRecorder::recordDrawCall(VulkanRenderer& renderer, VulkanCommandBuffer& commandBuffer, const VulkanPipeline& graphicsPipeline, const CPUDrawCallData data, ResourceManager& resourceManager, VkDescriptorSet currentUBO) {
	std::array<VkDescriptorSet, 4> descriptor{ currentUBO,resourceManager.getDescriptor(data.mat->resourceId).getDescriptorSet() ,resourceManager.getLightDescriptor().getDescriptorSet(),resourceManager.getPrefilteredCubeDescriptor().getDescriptorSet()};
